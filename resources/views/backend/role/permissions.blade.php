...
@section('content')
    ...
    <div class="md-modal md-effect-8 md-hide" id="modal-9">
        <div class="md-content">
            <div class="modal-header">
                <button class="md-close close">×</button>
                <h4 class="modal-title">渠道修改</h4>
            </div>
            <div class="modal-body">
                <form role="form" id="modifypower" action='{{url('backend/role/modifypower')}}' method="post">
                    {{ csrf_field() }}
                    <div class="form-group">
                        <label for="exampleInputEmail1">权限名称</label>
                        <input class="form-control name" name="name" placeholder="权限唯一英文名称" type="text">
                    </div>
                    <input id="power_id" type="hidden" name="power_id">
                    <div class="form-group">
                        <label for="exampleInputPassword1">可读名称</label>
                        <input class="form-control display_name" name="display_name" placeholder="权限可读中文名称" type="text">
                    </div>
                    <div class="form-group">
                        <label for="exampleTextarea">权限描述</label>
                        <textarea class="form-control description" id="description" name="description" rows="3" placeholder="权限相关描述信息"></textarea>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button type="button" id="form-submit" class="btn btn-primary modifypower">确认提交</button>
            </div>
        </div>
    </div>
    <div class="md-overlay"></div>
