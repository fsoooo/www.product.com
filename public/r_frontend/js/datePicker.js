...
window.datePicker = (function() {
    var MobileCalendar = function() {
        ...
        this.minM = 1;
        this.minD = 1;
        this.maxY = new Date().getFullYear();
        this.maxM = new Date().getMonth()+1;
        this.maxD = new Date().getDate();
        this.value="";
    };
...
;(function (undefined) {
	...
	function Schedule (opt) {
		var cssHtm = '#schedule-box{position:absolute;width:320px;margin:0 auto;padding:20px;font-size:13px;background:#fff;box-shadow:2px 2px 15px 2px #e3e3e3}#schedule-box .schedule-hd{position:relative;padding:0 15px}.schedule-left{position:absolute;top:0;left:0}.schedule-right{position:absolute;top:0;right:0}#schedule-box .today{text-align:center}#schedule-box .ul-box{overflow:hidden}#schedule-box .ul-box > li{float:left;width:14.28%;text-align:center;padding:5px 0}#schedule-box .other-month{color:#999999}#schedule-box .current-month{color:#333333}#schedule-box .today-style{border-radius:50%;background:#58d321}#schedule-box .arrow{margin-right:10px;font-weight:bold;cursor:pointer}#schedule-box .dayStyle{display:inline-block;width:35px;height:35px;border-radius:50%;text-align:center;line-height:35px;cursor:pointer}#schedule-box .current-month > .dayStyle:hover{background:#00BDFF;color:#ffffff}#schedule-box .today-flag{background:#00C2B1;color:#fff}#schedule-box .selected-style{background:#00BDFF;color:#ffffff}';
		    var cssEle = document.createElement("style");
			cssEle.type = "text/css";
			cssEle.appendChild(document.createTextNode(cssHtm));
			document.getElementsByTagName("head")[0].appendChild(cssEle);
		...
		$(ele).click(function(e){
			...
			var e = $(ele).get(0);
			var px = getLeft(e);
			var py = getTop(e)+e.offsetHeight;
			$('#schedule-box').css({'left':px+'px','top':py+'px'})
		});
	
		var bindEvent = function (){
			$('body').off().on('click',function(e){
				var target = $(e.target);
				if(target.is('#schedule-box') || target.is(ele) || target.parents('#schedule-box').length>0){
				}else{
					$('#schedule-box').hide();
				}	
			});
        
			$('#schedule-box').off().on('click',function(e){
				switch (e.target.id) {
					case 'nextMonth': 
						_this.nextMonthFun();
						break;
				...
				if(e.target.className.indexOf('currentDate') > -1){
					var dateVal = year+ '-' + (month+1)+ '-' +e.target.innerHTML;
					selectedDate = e.target.title;
					day = e.target.innerHTML;
					render();
					$(ele).val($('.today').text())
					$('#schedule-box').hide();
					opt.clickCb && opt.clickCb(dateVal);
				}
			});
		}
		var init = function () {
			var scheduleHd = '<div id="schedule-box"><div class="schedule-hd">'+
								'<div class="schedule-left">'+
									'<span class="arrow icon iconfont icon-zuoshuang" id="prevYear" ></span>'+
									'<span class="arrow icon iconfont icon-zuodan" id="prevMonth"></span>'+
								'</div>'+
								'<div class="today">'+formartDate(year,month+1,day,'-')+'</div>'+
								'<div class="schedule-right">'+
									'<span class="arrow icon iconfont icon-youdan" id="nextMonth"></span>'+
									'<span class="arrow icon iconfont icon-youshuang" id="nextYear"></span>'+
								'</div>'+
							'</div>'
			var scheduleWeek = '<ul class="week-ul ul-box">'+
									...
									'<li>六</li>'+
								'</ul>'
			var scheduleBd = '<ul class="schedule-bd ul-box" ></ul></div>'; 
			if(!$('#schedule-box').length){
				$('body').append(scheduleHd + scheduleWeek + scheduleBd);
			}else{
				$('#schedule-box').show();
			}
			bindEvent();
		...
		var render = function () {
			...
			document.getElementById('schedule-box').querySelector('.schedule-bd').innerHTML = eleTemp.join('');
			document.getElementById('schedule-box').querySelector('.today').innerHTML = formartDate(year,month+1,day,'-');
		};
