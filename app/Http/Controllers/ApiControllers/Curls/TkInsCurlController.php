...
class TkInsCurlController
{
    ...
    // 理赔接口地址
    const API_CLAIM_PRODUCT = 'http://ecuat.taikang.com/tkchannel_ydwy/service/yundaClaim';
    // 理赔接口地址(生产)  TODO  还没更新地址
    const API_CLAIM = 'http://channel.tk.cn/tkchannel/service/yundaClaim';
...
//        print_r($return);die;
    ...
    public function contractIns()
    {
        ...
        //中文转译问题JSON_UNESCAPED_UNICODE** 泰康接口不用考虑中文转码问题
        $sign = md5(self::INS_ENCODE_KEY . json_encode($data['apply_content']));
        $data['sign'] = $sign;
        //请求
    ...
    public function contractCallBack()
    {
        ...
        LogHelper::logSuccess($return, 'Tk_contract_call_back');
...
//        }
    ...
    public function issue()
    {
        ...
        $data['apply_content'] = [
            ...
            "payTime"=> $order->pay_time,    //支付时间
            "payMoney"=> $order->total_premium / 100,   //支付金额
            "payWayId"=> "72", //支付方式 微信支付
            "fromId"=> "64090", //渠道代码
            "comboId"=> $this->bind->p_code    //方案代码
        ];
        $sign = md5(self::INS_ENCODE_KEY_PRODUCT . json_encode($data['apply_content'])); //中文转译问题JSON_UNESCAPED_UNICODE** 泰康接口不用考虑中文转码问题
        ...
        try {
            ...
            switch ($status) {
                case 'pay_end':
                    ...
                    $order->pay_code = $result['tradeId']; //支付流水号
                    $order->pay_time = $result['pay_time']; //支付时间
                    $order->status = 'pay_end';
                    $order->save();
                    //查佣金比，统计财务
                    $by_stages_way = preg_replace('/[^0-9]+/', '', $order->by_stages_way);
