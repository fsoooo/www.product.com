...
//                $where['age'] = $age_y-$age_m;
    ...
    public function buyIns()
    {
        ...
        //非必传
        $holder_card_start_date = '';//证件有效期（起）
        $holder_card_end_date = '';//证件有效前（止）
		...
		$InsuredSex = $data['policyInsuredList'][0]['InsuredSex'];
//		if($InsuredSex=='1'){
//			 $InsuredSex = '0';
//		}elseif(InsuredSex){
//			$InsuredSex='1';
//		}
        ...
        if($InsuredRelation=='00'){//投被保人是同一人时，姓名，身份证，性别，手机号必须一致
            if($holder_card_no!=$InsuredCardNo|| $holder_email!=$InsuredEmail||$holder_mobile!=$InsuredMobile){
                $msg['data'] = '被保人是投保人本人，投被保人信息不一致！';
                $msg['code'] = 405;
                return $msg;
            }
        }else{//投被保人不是同一人时，手机号和邮箱不能相同
            if($holder_card_no==$InsuredCardNo|| $holder_email==$InsuredEmail||$holder_mobile==$InsuredMobile){
                $msg['data'] = '被保人非投保人本人，投保信息不能相同！';
                $msg['code'] = 405;
                return $msg;
            }
        }
        ...
        // //受益人信息  todo 当受益人为法定的时候不需要传这个节点与里面的内容，受益人非法定需要传，默认是法定的
